...
/*
  toString() {
    ...
    this.forEach((value, i) => {
      components.push(
        `{"real":${value.real.toFixed(2)}, "imag":${value.imag.toFixed(2)}}`
      );
    });

    return `{"fft": [${components.join(', ')}]}`;
  }
*/
  forEach(iterator) {
    const n = this.length;
  ...
  toString() {
    ...
    this.forEach((value, i) => {
      components.push(
        `{"mgn":${Math.sqrt(value.real * value.real + value.imag * value.imag)}}`
      );
    });
...
module.exports = function (RED) {
  function FestoFftNode(config) {
    ...
    node.on('input', function (msg) {      
      const data = new ComplexArray(msg.payload.length).map((value, i, n) => {
        value.real = msg.payload[i];
      });
      data.FFT();
      const output = data.magnitude();
